...
def substituir_valores_ausentes(df, colunas_rubricas, metodo='zero'):
    """Substituir valores ausentes por média, mediana ou zero nas colunas de rubricas."""
    if metodo == 'media':
        ...
        df[colunas_rubricas] = df[colunas_rubricas].fillna(0)
    return df
...
def normalizar_dados(df, colunas_rubricas):
    ...
    df = substituir_valores_ausentes(df, colunas_rubricas, metodo='zero')  # Ou 'mediana'
    return df, scaler

def normalizar_dados2(df, colunas_rubricas):
    # Cria o RobustScaler e normaliza as colunas de rubricas
    scaler = RobustScaler()
    df[colunas_rubricas] = scaler.fit_transform(df[colunas_rubricas])
    return df, scaler    
